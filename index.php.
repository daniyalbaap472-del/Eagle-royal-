...
<head>
    ...
    <style>
        ...
        .upcoming { background-color: #3498db; color: white; }
        .footer {
        ...
        .player-container {
            ...
            border-radius: 10px;
            text-align: center;
            /* New Sticky Player styles */
            position: sticky; /* Make the player stick to the screen */
            top: 0;
            z-index: 999; /* Ensure it stays on top of other content */
        }
        .player-placeholder {
            /* This div takes up space so content doesn't jump */
            display: none;
            height: 500px; /* This height should match the player container's height on most devices */
        }
        #player-section media-player {
            width: 100%;
            max-width: 900px;
            margin: 0 auto;
            border-radius: 8px;
        }
        /* Style for the "Back to Matches" button */
        #back-to-matches-btn {
            background-color: #007bff;
            color: white;
            border: none;
            padding: 12px 24px;
            margin-top: 20px; /* Added space below the player */
            border-radius: 5px;
            cursor: pointer;
            font-size: 1em;
            font-weight: bold;
...
<body>
    ...
    <div id="sidebar" class="sidebar">
        <a href="javascript:void(0)" class="sidebar-close" onclick="toggleNav()">×</a>
        <a href="#top" class="sidebar-nav-item">🏠 Home</a>
        <a href="#live-matches-section" class="sidebar-nav-item">🔥 Live Matches</a>
        <a href="#upcoming-matches-section" class="sidebar-nav-item">📅 Schedule</a>
        <a href="https://t.me/EAGLEROYALM3u8" target="_blank" class="sidebar-nav-item">✈️ Telegram Channel</a>
    </div>

    <div class="player-placeholder" id="player-placeholder"></div>
    <div id="player-section" class="player-container" style="display:none;">
        <media-player id="video-player" playsinline title="Eagle Royal Live" poster="https://files.vidstack.io/sprite-fight/poster.webp">
            <media-provider></media-provider>
            <media-video-layout></media-video-layout>
        </media-player>
        <button id="back-to-matches-btn" onclick="hidePlayer()">Back to Matches</button>
    </div>

    <div class="container">
        <hr>
        <h2 class="section-title" id="live-matches-section">Live Matches</h2>
        <div id="live-matches-container" class="match-grid">
            <p>Loading live matches...</p>
        </div>
        
        <hr>
        <h2 class="section-title" id="upcoming-matches-section">Upcoming Matches</h2>
        <div id="upcoming-matches-container" class="match-grid">
            <p>Loading upcoming matches...</p>
        </div>
    </div>

    <footer class="footer">
        <p>Join our Telegram Channel: <a href="https://t.me/EAGLEROYALM3u8" target="_blank">EAGLEROYALM3u8</a></p>
    </footer>

    <script>
        // Directly embed the JSON data for a single-file solution
        const apiData = {
    ...
    "matches": [
        ...
        function createMatchCard(match) {
            ...
            let liveButtons = '';
            if (match.status === 'LIVE' && match.streaming_options && match.streaming_options.length > 0) {
                liveButtons = match.streaming_options.map(option => {
                    return `<button class="watch-btn" onclick="playMatch('${option.url}', event)">Watch Now (${option.label})</button>`;
                }).join('');
                liveButtons = `<div class="watch-btn-group">${liveButtons}</div>`;
            } else if (match.status === 'LIVE' && match.STREAMING_CDN && match.STREAMING_CDN.Primary_Playback_URL) {
                // Fallback for matches with a single STREAMING_CDN object
                liveButtons = `<button class="watch-btn" onclick="playMatch('${match.STREAMING_CDN.Primary_Playback_URL}', event)">Watch Now</button>`;
            } else if (match.status === 'NOT_STARTED') {
                liveButtons = `<p class="upcoming-text">Coming Soon</p>`;
            }

            matchCard.innerHTML = `
                <img src="${match.image}" alt="${match.title}" class="match-image">
                <div class="match-content">
                    <p class="match-info">${match.tournament}</p>
                    <h3 class="match-title">${match.title}</h3>
                    <p class="match-info">Language: ${match.language}</p>
                    <p class="match-info">Start Time: ${match.startTime}</p>
                    <div class="teams-list">
                        ${teams}
                    </div>
                    <p class="match-info">Status: <span class="status-badge ${statusClass}">${match.status}</span></p>
                    ${liveButtons}
                </div>
            `;
            return matchCard;
        ...
        async function playMatch(url, event) {
            event.preventDefault(); // Prevent default link behavior
            
            // Hide match lists and show the player
            ...
            // Ensure Vidstack is loaded and ready
            await customElements.whenDefined('media-player');
            const mediaProvider = playerElement.querySelector('media-provider');

            if (Hls.isSupported() && url.endsWith('.m3u8')) {
                // If there's an existing HLS instance, destroy it first
                if (hlsInstance) {
                    hlsInstance.destroy();
                }
                ...
                hlsInstance.attachMedia(mediaProvider.media);
        ...
        function hidePlayer() {
            ...
            upcomingMatchesContainer.style.display = 'grid';
        }
        ...
        function renderMatches() {
            ...
            const liveMatches = apiData.matches.filter(m => m.status === 'LIVE');
            const upcomingMatches = apiData.matches.filter(m => m.status === 'NOT_STARTED');
